...
key=$(az storage account keys list  -n $storageacct -g $nrg -o json --query "[0].value" | tr -d \" )
secret_name=$(kubectl get secret -o json | jq ".items[] | select(.metadata.name | contains(\"$storageacct\")) | .metadata.name" | tr -d \")
